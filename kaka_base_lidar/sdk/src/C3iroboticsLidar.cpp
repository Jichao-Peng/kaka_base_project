...
C3iroboticsLidar::C3iroboticsLidar()
{
    ...
    m_enable_save_lidar_version_to_local = false;
    m_current_lidar_speed = -1.0;
    m_lidar_version_path = "/mnt/UDISK/config/lidar_version.ini";
    resetScanGrab();
}
...
TLidarGrabResult C3iroboticsLidar::analysisPacket(CLidarPacket &lidar_packet)
{
    ...
    if(!m_lidar_config.save_flag || lidar_packet.getLidarDeviceType() != m_lidar_config.type)
    {
        m_lidar_config.save_flag = true;
        m_lidar_config.type = lidar_packet.getLidarDeviceType();
        m_lidar_config.writeConfigFile(m_lidar_version_path);
    }
    ...
    switch(command_id)
    {
        ...
        case I3LIDAR_VERSION_INFO : return analysisLidarVersion(lidar_packet);
        default:
...
/***********************************************************************************
Function:     analysisLidarVersion
Description:  Analysis lidar version
Input:        None
Output:       None
Return:       None
Others:       None
***********************************************************************************/
TLidarGrabResult C3iroboticsLidar::analysisLidarVersion(CLidarPacket &lidar_packet)
{
    TLidarVersionInfo info = CLidarUnpacket::unpacketLidarVersion(lidar_packet);
    printf( "[C3iroboticsLidar] analysisLidarVersion sn(%s) version(%s)!\n",
              info.device_sn.c_str(), info.software_ver.c_str());
    if(m_enable_save_lidar_version_to_local)
    {
        m_lidar_config.device_sn = info.device_sn;
        m_lidar_config.software_ver = info.software_ver;
        m_lidar_config.type = lidar_packet.getLidarDeviceType();
        m_lidar_config.writeConfigFile(m_lidar_version_path);
    }
    return LIDAR_GRAB_ELSE;
...
/***********************************************************************************
Function:     getLidarReferenceSpeed
Description:  Get lidar refernce speed
Input:        None
Output:       None
Return:       None
Others:       None
***********************************************************************************/
double C3iroboticsLidar::getLidarReferenceSpeed()
{
    if(m_lidar_config.type == LDSBS)
    {
        return 6.0;     // 5.0
    }
    else
...
TLidarGrabResult C3iroboticsLidar::analysisHealthInfo(CLidarPacket &lidar_packet)
{
    ...
    printf("[C3iroboticsLidar] Lidar error is %5.5f!\n", lidar_error );
   	m_current_lidar_speed = lidar_error * 0.05f;
    m_receive_lidar_speed = true;
	 return LIDAR_GRAB_ERRO;
}
