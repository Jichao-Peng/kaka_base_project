...
CLidarPacketReceiver::TPacketResult CLidarPacketReceiver::processStateAcquireData(CLidarPacket *packet, u8 ch)
{
    ...
    {
        reset();
        if(packet->getAddressCode() == 1)
        {
            if(packet->verifyCheckSum(CLidarPacket::CHECK_NUMBER_SUM))
            {
//                packet->printHex();
            ...
            {
                printf( "[CLidarPacketReceiver] CRC verify wrong! CHECK_NUMBER_SUM\n");
                packet->printHex();
        ...
        {
            if(packet->verifyCheckSum(CLidarPacket::CHECK_NUMBER_CRC))
            {
                return PACKET_SUCCESS;
            }
            else
            {
                printf("[CLidarPacketReceiver] CRC verify wrong! CHECK_NUMBER_CRC\n");
                packet->printHex();
                return PACKET_FAILED;
            }
        }
