...
namespace everest
{
	namespace hwdrivers
	{
        ...
        struct TLidarConfig
        {
            ...
            {
                type         = LDSUNKNOW;
                device_sn    = "LIDAR_SN_UNKNOW";
                software_ver = "LIDAR_VERSION_UNKNOW";
                save_flag = false;
            }

            bool writeConfigFile(std::string file)
            {
                std::ofstream fo(file.c_str());
                if(fo)
                {
                    fo << (int)type << std::endl;
                    fo << device_sn << std::endl;
                    fo << software_ver << std::endl;
                    return true;
                }
                return false;
            }
            ...
            bool readConfigFile(std::string file)
            {
                ...
                if(fi)
                {
                    int read_type = LDSUNKNOW;
                    fi >> read_type;
                    fi >> device_sn;
                    fi >> software_ver;

                    type = (TLidarDeviceType)read_type;
                    return true;
                }
                return false;
            }

            TLidarDeviceType type;
		...
		class C3iroboticsLidar
		{
            public:
                ...
                /* Enable save lidar version to local */
                void enableSaveLidarVersionToLocal(bool enable) {m_enable_save_lidar_version_to_local = enable;}

                /* Set lidar version file path */
                void setLidarVersionFilePath(std::string file_path) {m_lidar_version_path = file_path;}

                double getLidarReferenceSpeed();
            ...
            private:
                ...
                /* Analysis new tooth scan */
                TLidarGrabResult analysisLidarVersion(CLidarPacket &lidar_packet);
            ...
            private:
                ...
                bool                    m_enable_save_lidar_version_to_local;
                std::string             m_lidar_version_path;
                double                  m_current_lidar_speed;

                CCountDown              m_data_count_down;
                TLidarConfig            m_lidar_config;
		};
	}
}
